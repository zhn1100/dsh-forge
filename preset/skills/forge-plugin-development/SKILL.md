...
- **Hard dependencies must be injected**: activation is concurrent; a row without `inject` may run before its services exist, and reading a missing service through `ctx.get` with a graceful-degradation fallback turns a boot failure into silent dead behavior. Declare hard deps in `inject` and fail loudly when a required service is absent.
- **Dynamic and formal tool schemas speak different dialects**: attribute-level `required: true` accepted by the dynamic tool registration is rejected by the formal `tools.register` validator. Revalidate every schema against the formal validator when porting a dynamic prototype.
